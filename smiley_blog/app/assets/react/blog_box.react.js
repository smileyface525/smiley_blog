...
var BlogBox = React.createClass({
    ...
    if (this.state.showDetail === true && this.state.blogDetail !== undefined) {
      return (
        <div>
          ...
          <a href="#" onClick={this.showList}>Back to list</a>
        </div>
      )
    }
  ...
  header: function() {
    return (
      <header className="absolute m0 center full-width top-0 bg-orange">
        { this.state.loggedIn ? <Navbar /> : <LoginForm loginFailed={this.state.loginFailed} /> }
        <h1 className="bold">sm:)eyblog</h1>
      </header>
    )
  }
